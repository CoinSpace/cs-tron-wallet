...
export default class TronWallet extends CsWallet {
  ...
  constructor(options = {}) {
    ...
    this.#loadTransactions = this.memoize(this._loadTransactions);
  }
  ...
  async cleanup() {
    ...
    this.memoizeClear(this.#loadTransactions);
  }
  ...
  async _getChainParameters() {
    return this.#api.chainpaPameters();
  }
  ...
  async _loadTransactions({ cursor }) {
  ...
  async loadTransactions({ cursor } = {}) {
    ...
    return this.#loadTransactions({ cursor });
  }
  ...
  async loadTransaction(id) {
    ...
    while (hasMore) {
      const data = await this.#loadTransactions({ cursor });
      ({ hasMore, cursor } = data);
