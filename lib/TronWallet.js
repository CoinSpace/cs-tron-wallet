...
class TronTransaction extends Transaction {
  get url() {
    if (this.development) {
      return `https://nile.tronscan.org/#/transaction/${this.id}`;
    }
    return `https://tronscan.org/#/transaction/${this.id}`;
  }
}
...
export default class TronWallet extends CsWallet {
  ...
  async #getCoinBalance() {
    const { balance } = await this.requestNode({
      url: `api/v1/account/${this.#addressString}/balance`,
      method: 'GET',
    });
  ...
  async #getTokenBalance() {
    const { balance } = await this.requestNode({
      url: `api/v1/account/${this.#addressString}/trc20/${this.crypto.address}/balance`,
      method: 'GET',
    });
    return BigInt(balance || 0);
  }
  ...
  async _getChainParameters() {
    const parameters = await this.requestNode({
  ...
  async _getMinerFee({ address, value }) {
    ...
    if (this.crypto.type === 'coin') {
      const destinationAccount = await this.requestNode({
        url: `api/v1/account/${address.toBase58Check()}`,
        method: 'GET',
      });
      if (!destinationAccount) {
        return feeLimit;
      }
      const resources = await this.requestNode({
        url: `api/v1/account/${this.#addressString}/resources`,
        method: 'GET',
      });
    ...
    if (this.crypto.type === 'token') {
      const { energy } = await this.requestNode({
        url: `api/v1/estimateenergy/${this.crypto.address}`,
        method: 'GET',
        params: {
          from: this.#addressString,
          to: address.toBase58Check(),
          value,
        },
      });
  ...
  async createTransaction({ address, amount }, seed) {
    ...
    const res = await this.requestNode({
      ...
      method: 'POST',
      data: {
        transaction: hex.encode(transaction.serialize()),
      },
    });
    if (res?.code === 'SUCCESS') {
      if (this.crypto.type === 'coin') {
  ...
  async loadTransactions({ cursor } = {}) {
    const url = this.crypto.type === 'token'
      ? `api/v1/account/${this.#addressString}/trc20/${this.crypto.address}/transactions`
      : `api/v1/account/${this.#addressString}/transactions`;
    const res = await this.requestNode({
      url,
      method: 'GET',
      params: {
    ...
    return {
      transactions: this.#transformTxs(res.data),
  ...
  #transformTRC20Tx(tx) {
    ...
    return new TronTransaction({
      type: Transaction.TYPE_TRANSFER,
      //status: tx.ret[0].contractRet === 'SUCCESS' ? Transaction.STATUS_SUCCESS : Transaction.STATUS_FAILED,
      id: tx.transaction_id,
      amount: new Amount(tx.value, this.crypto.decimals),
      incoming,
      from: tx.from,
      to: tx.to,
      fee: new Amount(0, this.platform.decimals),
      timestamp: new Date(tx.block_timestamp),
      confirmations: 0,
      minConfirmations: 0,
      development: this.development,
